...
КОМАНДА /help:
- Когда пользователь отправляет команду "/help", ОБЯЗАТЕЛЬНО используй инструмент rag_query
- Параметры для rag_query: query="environment variables configuration", topN=10, threshold=0.0, initialTopK=20
- threshold=0.0 означает что нужно показать ВСЕ найденные чанки, даже с низкой релевантностью
- После получения результатов от rag_query, отформатируй их четко и структурированно
- Покажи все переменные окружения и их назначение из полученных чанков
- Если rag_query вернул пустой результат (results: []), только тогда скажи что нужно проиндексировать файл через POST /api/pdf/process
- НЕ используй другие инструменты кроме rag_query для команды /help
`;
  }
}
