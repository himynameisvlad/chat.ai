...
### 3. Настройка RAG (настоятельно рекомендуется)

RAG (Retrieval-Augmented Generation) значительно улучшает качество ревью, предоставляя контекст из документации проекта.

**Что даст RAG:**
- ✅ Проверка соответствия изменений архитектуре проекта
- ✅ Валидация environment переменных через .env.example
- ✅ Ссылки на существующие паттерны и best practices
- ✅ Контекстно-зависимые рекомендации
- ✅ Проверка consistency с документированными стандартами
...
RAG автоматически извлекает:
- `.env.example` - для валидации переменных окружения
- README и другие `.md` файлы - для понимания проекта
- Код из diff - для поиска похожих паттернов

Для лучших результатов проиндексируйте всю документацию проекта:
```bash
# Если есть скрипт инициализации
npm run rag:init
```

**Как работает RAG в PR Review:**

При анализе PR система делает 3 запроса к RAG:
1. **Код-контекст**: Извлекает ключевые слова из diff (имена файлов, функций, классов, импорты) и ищет релевантную документацию
2. **.env.example**: Всегда загружает для проверки environment переменных
3. **README/Docs**: Получает общие guidelines и best practices проекта

Все найденные документы передаются LLM для анализа.
